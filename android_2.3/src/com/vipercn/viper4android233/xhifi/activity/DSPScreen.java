...
public final class DSPScreen extends PreferenceActivity
{
		...
		public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key)
		{
			...
			if ("viper4android.headphone_xhifi.fireq".equals(key))
			{
				...
				if (!"custom".equals(pref))
				{
					...
					e.putString("viper4android.headphone_xhifi.fireq.custom", pref);
					e.commit();
			...
			{
				String customSetting = sharedPreferences.getString("viper4android.headphone_xhifi.fireq.custom", null);
				...
				if (!desiredValue.equals(sharedPreferences.getString("viper4android.headphone_xhifi.fireq", null)))
				{
					Editor e = sharedPreferences.edit();
	...
	{
		final String szSharedPref = ViPER4Android_XHiFi.SHARED_PREFERENCES_BASENAME + "." + getSubPage();
        ...
        case R.id.loadprofile:
        {
            ...
            .setItems(arrayProfileList, new DialogInterface.OnClickListener()
            {
            	...
            	{
            		String szProfilePath = Environment.getExternalStorageDirectory() + "/ViPER4Android/ProfileXHiFi/";
					Log.i("ViPER4Android_XHiFi", "Load effect profile, current sharedPref = " + szSharedPref);
							...
							else
							{
								...
								if (Utils.CheckProfileExists(szProfileName, Environment.getExternalStorageDirectory() + "/ViPER4Android/ProfileXHiFi/"))
								{
				        			...
				        			mConfirm.setPositiveButton(getResources().getString(R.string.text_yes), new DialogInterface.OnClickListener()
				        			{
										...
										public void onClick(DialogInterface dialog, int which)
										{
											Log.i("ViPER4Android_XHiFi", "Save effect profile, current sharedPref = " + szSharedPref);
											Utils.SaveProfile(szSaveProfileNameGlobal, Environment.getExternalStorageDirectory() + "/ViPER4Android/ProfileXHiFi/", szSharedPref, mActivityContext);
								...
								Log.i("ViPER4Android_XHiFi", "Save effect profile, current sharedPref = " + szSharedPref);
								Utils.SaveProfile(szProfileName, Environment.getExternalStorageDirectory() + "/ViPER4Android/ProfileXHiFi/", szSharedPref, mActivityContext);
	...
	{
		return super.getSharedPreferences(ViPER4Android_XHiFi.SHARED_PREFERENCES_BASENAME + "." + getSubPage(), mode);
	}
